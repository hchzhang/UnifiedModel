%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [bestx,bestf] = sceua(x0,bl,bu,maxn,kstop,pcento,peps,ngs,iseed,iniflg,idss,nsite,site,mete,phen)
...
icall=0;
for i=1:npt;
    xf(i) = functnphen(nopt,x(i,:),idss,nsite,site,mete,phen);
    icall = icall + 1;
end;
f0=xf(1);
...
while icall<maxn & gnrng>peps & criter_change>pcento;
    ...
    for igs = 1: ngs;
        ...
        for loop=1:nspl;
            ...
            [snew,fnew,icall]=cceua(s,sf,bl,bu,icall,maxn,idss,nsite,site,mete,phen);
