...
DD90s(:,1:nvar)=xlsread([inpath,'U2_StartDay_80s_aveDurations.xlsx'],'Result','K2:N6990');
sigDD90s(:,1:nvar)=xlsread([inpath,'U2_StartDay_80s_aveDurations.xlsx'],'Result','CY2:DB6990');
DD10s(:,1:nvar)=xlsread([inpath,'U2_StartDay_80s_aveDurations.xlsx'],'Result','W2:Z6990');
sigDD10s(:,1:nvar)=xlsread([inpath,'U2_StartDay_80s_aveDurations.xlsx'],'Result','DS2:DV6990');
...
varname={'?T_{Winter}','?T_{Spring}','?MAT','?T_{Preseason}'};
...
for ipr=1:nperiod
    ...
    for ivar=1:nvar
        ...
        nlevel=13;
        level=[-999 -4.0 -3.0 -2.0 -1.0 -0.5 -0.1 0.1 0.5 1.0 2.0 3.0 4.0 999];
        ...
        if (ipr==nperiod && ivar==3)
            ...
            set(hc1,'xTick',0.99:0.93:13,'xTickLabel',{'',level(2:6),'<0','>0',level(9:13),''},'fontsize',fsize+2);
            hc1.Position=[0.17,0.095,0.7,0.02];
            text(lonw+2-1.6,lats-7.4,'Change in temperature (^oC)','FontSize',fsize+3,'horizontalalignment','center','Rotation',0);
            % colormap(color);
        end
        clear MAPcolor;
...
print figure
print(fig,'-dtiff','-r300',[outpath,'MAPs_Tem_changes']);
box off;
% close;
